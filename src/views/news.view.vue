  ...
  <div class="wrapper my-6 md:my-10 grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-8">
    ...
    <div class="lg:col-span-2 space-y-6 md:space-y-10">
      ...
      >
        <div class="p-4 sm:p-6">
          ...
          <h2 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-900 mb-2">{{ post.title }}</h2>
          <p class="text-gray-600 text-sm sm:text-base mb-4">{{ post.description }}</p>
          <img :src="post.image" class="w-full h-40 sm:h-56 md:h-64 object-cover rounded-lg mb-4" />
          <Button variant="default" class="flex items-center gap-2 w-full sm:w-auto">
    ...
    <div class="space-y-4 md:space-y-8 mt-8 lg:mt-0">
      ...
      <div class="bg-white rounded-xl shadow p-3 sm:p-4">
        <h3 class="font-semibold text-base sm:text-lg mb-2 sm:mb-4">Chuyên mục</h3>
        <ul class="space-y-1 sm:space-y-2">
          <li v-for="(cat, i) in categories" :key="i">
            <Button variant="ghost" class="justify-between w-full text-xs sm:text-sm">
      ...
      <!-- Bài viết mới nhất -->
      <div class="bg-white rounded-xl shadow p-3 sm:p-4">
        <h3 class="font-semibold text-base sm:text-lg mb-2 sm:mb-4">Bài viết mới nhất</h3>
        <div class="space-y-2 sm:space-y-4">
          ...
          >
            <img :src="recent.thumb" class="w-12 h-12 sm:w-[60px] sm:h-[60px] rounded object-cover" />
            <div>
              <p class="font-semibold text-xs sm:text-sm leading-snug">{{ recent.title }}</p>
              <p class="text-[10px] sm:text-xs text-muted-foreground">{{ recent.date }}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <!-- Từ khóa phổ biến -->
      <div class="bg-white rounded-xl shadow p-3 sm:p-4">
        <h3 class="font-semibold text-base sm:text-lg mb-2 sm:mb-4">Từ khóa phổ biến</h3>
