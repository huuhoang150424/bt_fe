...
<div class="bg-white rounded-xl shadow-md p-6 space-y-4 lg:col-span-3">
  <h3 class="text-xl font-semibold text-violet-600 flex items-center gap-2">
    <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-violet-600" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8h2a2 2 0 012 2v8a2 2 0 01-2 2H7l-4 4V10a2 2 0 012-2h2" />
    </svg>
      ...
      <div class="flex gap-4">
        ...
        <div>
          <p class="text-gray-500 text-sm flex items-center gap-2">
            <CalendarDaysIcon class="w-4 h-4 text-violet-500" /> {{ comment.date }}
          </p>
          <h4 class="font-semibold text-gray-800">{{ comment.name }}</h4>
          <p class="text-gray-700 mt-1">{{ comment.content }}</p>
          <button class="text-sm text-violet-600 font-medium mt-2 hover:underline">↩ Reply</button>
        </div>
      </div>
    </div>
  </div>
...
<div class="bg-white rounded-xl shadow-md p-6 space-y-4 lg:col-span-3">
  ...
  <p class="text-gray-500 text-sm">Địa chỉ email của bạn sẽ không được công bố. Các trường bắt buộc được đánh dấu *</p>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    ...
    <div class="relative">
      ...
      />
      <svg class="absolute top-4 left-3 w-5 h-5 text-gray-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
  ...
  <div class="flex items-center space-x-2">
    <input type="checkbox" id="remember" v-model="commentForm.remember" class="accent-violet-600" />
    <label for="remember" class="text-sm text-gray-700">Lưu tên và email của tôi trong trình duyệt này cho lần bình luận tiếp theo.</label>
  </div>
