...
<template>
  ...
  <div class="container mx-auto py-12 px-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
      ...
      >
        <h3 class="text-2xl font-bold mb-4 text-gray-800 group-hover:text-white">{{ plan.name }}</h3>
        <p class="text-gray-600 mb-6 group-hover:text-white">{{ plan.description }}</p>
        <p class="text-4xl font-bold text-violet-700 mb-6 group-hover:text-white">{{ plan.price }}</p>
        <ul class="text-gray-700 text-left mb-8 group-hover:text-white">
          <li v-for="(feature, fIndex) in plan.features" :key="fIndex" class="mb-2 flex items-center">
            <svg class="w-5 h-5 mr-2 text-green-500 group-hover:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
            {{ feature }}
          </li>
        </ul>
        <Button class="w-60 h-12 bg-blue-600 hover:bg-violet-700 text-white font-bold py-3 px-8 rounded-full transition-colors duration-300 group-hover:bg-white group-hover:text-violet-700">
          MUA NGAY
        </Button>
      </div>
    </div>
    ...
    <div class="relative bg-cover bg-center py-20 px-4 rounded-lg overflow-hidden mb-20"
      ...
      <div class="relative z-10 text-center text-white">
        ...
        <p class="text-lg mb-8">Chúng tôi luôn sẵn lòng giải đáp mọi thắc mắc của bạn.</p>
        <Button class="w-40 h-26 bg-violet-600 hover:bg-violet-700 text-white font-bold py-3 px-8 rounded-full focus:outline-none focus:shadow-outline">
          LIÊN HỆ NGAY
        </Button>
      </div>
    </div>
